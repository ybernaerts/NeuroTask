...
def fit(seqs_train, x_dim=3, bin_width=20.0, min_var_frac=0.01, em_tol=1.0E-8,
    ...
    params_init['M'] = fa.components_.T
    params_init['U'] = fa.noise_variance_
...
def em(params_init, seqs_train, max_iters=500, tol=1.0E-8, min_var_frac=0.01,
    ...
    """
    Fits SGPFA model parameters using expectation-maximization (EM) algorithm.

    Parameters
    ----------
    params_init : dict
        SGPFA model parameters at which EM algorithm is initialized
        covType : {'rbf', 'tri', 'logexp'}
            type of GP covariance
        gamma : np.ndarray of shape (latents)
            related to GP timescales by
            'bin_width / sqrt(gamma)'
        eps : np.ndarray of shape (latents)
            GP noise variances
        d : np.ndarray of shape (neurons)
            observation mean
        R : np.ndarray of shape (neurons, neurons)
            observation noise covariance
        M : np.ndarray of shape (neurons, latents)
            matrix normal mean
        U : np.ndarray of shape (neurons, neurons)
    ...
    -------
    params_est : dict
        SGPFA model parameter estimates, returned by EM algorithm (same
    ...
    for iter_id in trange(1, max_iters + 1, desc='EM iteration',
        ...
        if not np.isnan(ll):
            ll_old = ll
        seqs_latent, mn, ll = exact_inference_with_ll(seqs_train, params,
                                                  get_ll=get_ll)
        lls.append(ll)
        ...
        d = (1/t.sum())(y-M.dot(latent_variable)).sum(axis=1)
        params['d'] = d
                       
        # Observation noise covariance
        # ----------------------------
        # Note: we update R with already updated parameters U and M 
        # We first need <XXT>=Vsm+<X><X>T
        ...
        # From matrix normal properties
        cxxTcT = U*np.trace(sum_p_auto) + (M.dot(sum_p_auto)).dot(M.T)
        # Finally:
        r = cxxTcT - (M.dot(latent_variable.dot(latent_variable.T))).dot(M.T)
        
        # Alternatively, should be equivalent:
        Vsm = np.zeros((x_dim, x_dim))
        for seq_latent in seqs_latent:
            Vsm += seq_latent['Vsm'].sum(axis=2)                     
        r = U*np.trace(sum_p_auto) + (M.dot(Vsm)).dot(M.T)
                       
        if params['notes']['RforceDiagonal']:
            # set minimum private variance floor
            R = np.diag(np.maximum(var_floor,np.diag((1/t.sum())*r)))
            params['R']=R
        else:
            r =  (1/t.sum())*r
...
def exact_inference_with_ll(seqs, params, get_ll=True):
    """
    Extracts latent trajectories and matrix normal expectancies from neural data, given GPFA model parameters.
    ...
    mn : np.recarray
        ...
        C : (neurons, latents) np.ndarray
            posterior matrix normal mean
        CCT: (neurons, neurons) np.ndarray
            posterior matrix normal row-covariance
    ll : float
